...
- S8 loads a photo via `s0.initImage(url)`. URL must be CORS-friendly — `images/web/` in this repo is served via GitHub raw, which works. Hot-swap mid-set by sweeping S8's middle knob (CC 36 — Send B on column 8) or calling `useImage(n)` from the console. The image list lives in the `images` array at the top of `vj-patch.js`. NB: column 8 breaks the column-N knob convention because of this — Send B on col 8 is image select, not "grit".
